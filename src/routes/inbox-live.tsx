...
function isLidSender(value: string | null): boolean { return Boolean(value && /^\d{10,18}$/.test(value) && !value.startsWith('62')) }
function displayPhone(value: string | null): string { return isDisplayablePhone(value) ? value ?? '' : '' }
function displayName(contact: Contact | undefined, phone: string | null): string { return contact?.name || displayPhone(contact?.phoneNumber ?? phone) || (isLidSender(contact?.phoneNumber ?? phone) ? 'Pengunjung PST' : '') }
...
function InboxLivePage() {
  ...
  return (
    <div class={isLight() ? 'fixed inset-0 h-screen w-screen overflow-hidden bg-[#d9dbd5] text-[#111b21]' : 'fixed inset-0 h-screen w-screen overflow-hidden bg-[#0c1317] text-[#e9edef]'}>
...
function StatusIcon(props: { status: string }) { return props.status === 'failed' ? <svg class="h-3.5 w-3.5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg> : <svg class="h-[15px] w-[22px] text-[#53bdeb]" viewBox="0 0 22 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 8.5l4 4L13 3.5"/><path d="M6 8.5l4 4L18 3.5"/></svg> }
