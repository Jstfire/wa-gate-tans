    ...
    <div class={isLight() ? 'h-dvh overflow-hidden bg-[#f0f2f5] text-[#111b21]' : 'h-dvh overflow-hidden bg-[#111b21] text-[#e9edef]'}>
      <div class="flex h-full w-full overflow-hidden">
        <aside class={`${selectedContact() ? 'hidden md:flex' : 'flex'} h-full w-full max-w-[480px] flex-col border-r md:flex md:w-[35%] lg:w-[30%] ${isLight() ? 'border-[#d1d7db] bg-white' : 'border-[#2a3942] bg-[#111b21]'}`}>
          <div class={`relative flex h-[64px] items-center justify-between px-5 ${isLight() ? 'bg-white' : 'bg-[#111b21]'}`}>
            <h1 class="text-[22px] font-semibold tracking-tight">Chats</h1>
            ...
            <SidebarMenu open={sidebarMenu()} light={isLight()} onClose={() => setSidebarMenu(false)} />
          </div>
          <div class={`space-y-2 p-2 ${isLight() ? 'bg-white' : 'bg-[#111b21]'}`}>
            ...
            <div class="flex gap-2 overflow-x-auto pb-1"><FilterPill active label="All" /><FilterPill label="Unread" /><FilterPill label="Favourites" /><FilterPill label="Groups" /></div><div class={`rounded-lg border p-2 ${isLight() ? 'border-[#e9edef] bg-[#f7f8fa]' : 'border-[#2a3942] bg-[#202c33]'}`}>
...
function MessageMenu(props: { open: boolean; message: Message; light: boolean; onAction: (message: Message, action: MessageAction) => void }) { return <Show when={props.open}><div class="absolute right-2 top-7 z-30 w-56"><div class={`mb-2 flex items-center gap-2 rounded-full px-2 py-1.5 shadow-xl ${props.light ? 'bg-white' : 'bg-[#233138]'}`}><For each={['👍', '❤️', '😂', '😮', '😢', '🙏', '+']}>{(emoji) => <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full hover:bg-black/10">{emoji}</button>}</For></div><div class={`overflow-hidden rounded-xl py-1 text-sm shadow-2xl ${props.light ? 'bg-white text-[#111b21]' : 'bg-[#233138] text-[#e9edef]'}`}><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'reply')}>Reply</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'copy')}>Copy</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'forward')}>Forward</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'detail')}>Pin</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'detail')}>Star</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'detail')}>Select</button><div class="my-1 border-t border-white/10" /><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={() => props.onAction(props.message, 'detail')}>Report</button><button class="block w-full px-4 py-2.5 text-left text-red-400 hover:bg-black/10" onClick={() => props.onAction(props.message, 'detail')}>Delete</button></div></div></Show> }
function SidebarMenu(props: { open: boolean; light: boolean; onClose: () => void }) { return <Show when={props.open}><div class={`absolute right-3 top-12 z-40 w-56 overflow-hidden rounded-xl py-2 text-sm shadow-2xl ${props.light ? 'bg-white text-[#111b21]' : 'bg-[#233138] text-[#e9edef]'}`}><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={props.onClose}>New group</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={props.onClose}>Starred messages</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={props.onClose}>Select chats</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={props.onClose}>Mark all as read</button><button class="block w-full px-4 py-2.5 text-left hover:bg-black/10" onClick={props.onClose}>App lock</button><a class="block px-4 py-2.5 hover:bg-black/10" href="/dashboard">Dashboard</a></div></Show> }
